...
export const ExhibitionView: React.FC<ExhibitionViewProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[90] isolate bg-stone-950 text-white animate-in fade-in duration-500 overflow-hidden pt-[env(safe-area-inset-top,0px)] pb-[env(safe-area-inset-bottom,0px)] pl-[env(safe-area-inset-left,0px)] pr-[env(safe-area-inset-right,0px)]">
      {/* Header - Absolutely positioned overlay with backdrop blur */}
      <header className="absolute top-0 left-0 right-0 py-4 px-6 sm:p-8 flex justify-between items-center backdrop-blur-md bg-stone-950/95 z-20">
        <div>
      ...
      {/* Content - Centered with natural height */}
      <div className="h-full flex items-center justify-center px-6 sm:px-12 py-24 sm:py-32">
        <div className="relative max-w-4xl w-full">
          {/* Navigation arrows positioned relative to content */}
          <button
            onClick={prev}
            className="absolute -left-4 sm:-left-20 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-16 sm:h-16 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all hover:scale-110 z-10"
          >
            <ChevronLeft size={24} className="sm:w-8 sm:h-8" />
          </button>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-16 items-center animate-in zoom-in-95 duration-700">
            <div className="aspect-[3/4] rounded-2xl sm:rounded-[3rem] overflow-hidden shadow-2xl border border-white/10 group relative mx-auto w-full max-w-sm sm:max-w-none">
              <ItemImage
                itemId={item.id}
          ...
          <button
            onClick={next}
            className="absolute -right-4 sm:-right-20 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-16 sm:h-16 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all hover:scale-110 z-10"
          >
      ...
      {/* Footer - Absolutely positioned overlay */}
      <footer className="absolute bottom-0 left-0 right-0 py-4 px-6 sm:p-12 flex justify-center backdrop-blur-md bg-stone-950/95 z-20">
          ...
          {collection.items.slice(0, 10).map((_, i) => (
            <button
              ...
              className={`h-1 rounded-full transition-all ${
                i === index ? 'w-8 sm:w-12 bg-amber-500' : 'w-2 sm:w-4 bg-white/10'
              }`}
            />
          ))}
          {collection.items.length > 10 && (
            <span className="text-[8px] opacity-30 self-center">+</span>
          )}
        </div>
      </footer>
    </div>
