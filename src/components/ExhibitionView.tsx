...
export const ExhibitionView: React.FC<ExhibitionViewProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[90] isolate bg-stone-950 text-white flex flex-col animate-in fade-in duration-500 overflow-hidden pt-[env(safe-area-inset-top,0px)] pb-[env(safe-area-inset-bottom,0px)] pl-[env(safe-area-inset-left,0px)] pr-[env(safe-area-inset-right,0px)]">
      <header className="py-4 px-6 sm:p-8 flex justify-between items-center bg-gradient-to-b from-stone-950 via-stone-950/90 to-transparent z-10 flex-shrink-0">
        <div>
      ...
      <div className="flex-1 min-h-0 relative flex flex-col sm:flex-row items-center justify-start sm:justify-center px-6 sm:px-12 pt-2 pb-4 sm:py-8 gap-4 sm:gap-16">
        <button
          onClick={prev}
        ...
        <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-16 items-center animate-in zoom-in-95 duration-700">
          <div className="aspect-[3/4] sm:aspect-[3/4] rounded-2xl sm:rounded-[3rem] overflow-hidden shadow-2xl border border-white/10 group relative max-h-[55vh] sm:max-h-none mx-auto w-full max-w-sm sm:max-w-none">
            <ItemImage
              itemId={item.id}
        ...
        <button
          onClick={next}
          className="absolute right-4 sm:right-12 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-16 sm:h-16 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all hover:scale-110 z-10"
        >
          <ChevronRight size={24} className="sm:w-8 sm:h-8" />
        </button>
      </div>

      <footer className="py-4 px-6 sm:p-12 flex justify-center bg-gradient-to-t from-stone-950 via-stone-950/90 to-transparent flex-shrink-0">
