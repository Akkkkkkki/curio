    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-stone-100">
          <h2 className="font-serif font-bold text-lg text-stone-800">New Collection</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Preset/Schema Section */}
          <div>
            <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">
              Data Presets
            </label>
            <p className="text-xs text-stone-400 mb-3">Determines which fields (e.g. Artist, Year) are available.</p>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
                {TEMPLATES.find(t => t.id === selectedTemplateId)?.fields.map(f => (
                    <span key={f.id} className="text-[10px] px-2 py-1 bg-stone-100 text-stone-500 rounded-md border border-stone-200">
                        {f.label}
                    </span>
                ))}
            </div>
