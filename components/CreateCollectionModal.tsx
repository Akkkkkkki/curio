...
export const CreateCollectionModal: React.FC<CreateCollectionModalProps> = ({
  ...
  const selectedTemplate =
    TEMPLATES.find((temp) => temp.id === selectedTemplateId) || TEMPLATES[0];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`flex items-center justify-between p-4 border-b ${dividerBorder}`}
        >
          <h2
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-6 pb-24 sm:pb-6 space-y-6">
          ...
          <div
            className={`p-4 rounded-2xl border ${theme === "vault" ? "border-white/10 bg-white/5" : "border-stone-100 bg-stone-50/60"}`}
          >
            ...
            <p
              className={`text-[12px] uppercase font-semibold ${mutedText} tracking-[0.12em] mb-2`}
            >
              {t("templateFields")}
            </p>
            <div className="grid grid-cols-2 gap-2">
              {selectedTemplate.fields.map((field) => (
                <div
                  key={field.id}
                  className={`flex items-center gap-2 px-2 py-1.5 rounded-lg border shadow-inner ${theme === "vault" ? "bg-white/5 border-white/10" : "bg-white border-stone-100"}`}
                >
                  <span
                    className={`text-[11px] font-bold uppercase tracking-[0.12em] ${mutedText}`}
                  >
