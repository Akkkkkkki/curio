...
export const Layout: React.FC<LayoutProps> = ({ children, onOpenAuth, onSignOut, onImportLocal, hasLocalImport = false, importState = 'idle', importMessage = null, user, isSupabaseConfigured, headerExtras }) => {
  ...
  const statusColor = !isSupabaseConfigured
    ? (theme === 'vault' ? 'text-stone-400' : 'text-stone-400')
    : isAuthenticated
      ? (theme === 'vault' ? 'text-emerald-300' : 'text-emerald-600')
      : (theme === 'vault' ? 'text-amber-200' : 'text-amber-600');
  const shellClass = theme === 'vault' ? 'text-white' : 'text-stone-800';
  const headerSurface = theme === 'vault'
                    ...
                    <div className={`absolute right-0 mt-2 w-72 max-w-[calc(100vw-2rem)] ${dropdownSurface} rounded-[1.5rem] shadow-2xl border p-2 animate-in slide-in-from-top-2 duration-200 z-50`}>
                        <div className={`p-4 border-b ${borderClass} mb-1`}>
                            ...
                            <div className="flex items-start gap-3 mt-3">
                                <div className={`p-2 rounded-xl ${isSupabaseConfigured ? (isAuthenticated ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600') : (theme === 'vault' ? 'bg-white/10 text-white/60' : 'bg-stone-50 text-stone-400')}`}>
                                    {statusIcon}
                                </div>
                        ...
                        <div className="px-4 pb-3 pt-1">
                          <p className="text-[11px] font-bold uppercase tracking-[0.14em] opacity-70 mb-2">{t('themeSelection')}</p>
                        ...
                        {hasLocalImport && isAuthenticated && onImportLocal && (
                            <div className="p-2 border-t border-stone-50">
                                <div className="p-3 rounded-xl border border-amber-100 bg-amber-50/60">
                                    <p className="text-[12px] font-bold text-amber-900 uppercase tracking-[0.18em] mb-1">{t('importLocalTitle')}</p>
                                    ...
                                    {importMessage && (
                                        <p className={`text-[12px] mt-2 ${importState === 'error' ? 'text-red-500' : 'text-amber-700'}`}>
                                            {importMessage}
                                        </p>
                                    )}
                                </div>
