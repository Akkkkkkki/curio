...
const optionMeta: Record<AppTheme, { labelKey: keyof typeof translations['en']; swatch: string[] }> = {
  gallery: { labelKey: 'themeGallery', swatch: ['bg-white', 'bg-stone-100'] },
  vault: { labelKey: 'themeVault', swatch: ['bg-stone-900', 'bg-stone-700'] },
  atelier: { labelKey: 'themeAtelier', swatch: ['bg-[#f5f1e7]', 'bg-[#e7dfd0]'] },
};
...
export const ThemePicker: React.FC<ThemePickerProps> = ({ layout = 'inline' }) => {
  const { theme, setTheme } = useTheme();
  const { t } = useTranslation();
  const isStacked = layout === 'stacked';
  const inlineSurface = theme === 'vault'
  ...
  const inactiveText = theme === 'vault' ? 'text-white/70 hover:text-white' : 'text-stone-500 hover:text-stone-800';
  const activeSurface = theme === 'vault' ? 'bg-white/10 text-white border border-white/20' : 'bg-white shadow-sm ring-1 ring-amber-200 text-stone-900';
  const stackedSurface = theme === 'vault' ? 'border-white/10 bg-white/5 text-white' : 'border-stone-200 bg-white text-stone-600';
        ...
        {(Object.keys(optionMeta) as AppTheme[]).map(opt => (
          <button
            ...
            className={`flex items-center justify-between w-full px-3 py-2 rounded-lg border transition-all text-sm font-semibold ${
              theme === opt ? 'border-amber-200 bg-amber-50 text-stone-900 shadow-sm' : stackedSurface
            }`}
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={`hidden sm:flex items-center gap-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${theme === 'vault' ? 'text-white/60' : 'text-stone-400'}`}>
        <Paintbrush size={14} />
        <span>{t('themeSelection')}</span>
      </div>
      <div className={`flex items-center gap-1 rounded-full px-1 h-10 border ${inlineSurface}`}>
        {(Object.keys(optionMeta) as AppTheme[]).map(opt => (
          <button
            key={opt}
            onClick={() => setTheme(opt)}
