...
export const AddItemModal: React.FC<AddItemModalProps> = ({
  ...
  const analyze = async (base64: string) => {
    ...
    try {
      ...
      const result = await analyzeImage(
        base64Data,
        currentCollection.customFields,
      );
  ...
  const renderStepper = () => (
    <div className="space-y-2 mb-4">
      <div
        className={`flex items-center justify-between text-[12px] ${mutedText}`}
      >
        <span className="font-semibold">{progressCopy}</span>
        <span className={`text-[12px] ${mutedText}`}>
          {stepItems[currentStepIndex]?.helper}
        </span>
      </div>
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <p className={`text-[12px] leading-tight ${mutedText}`}>
                  {s.helper}
                </p>
              </div>
            </div>
              ...
              <div className="flex-1 space-y-2">
                <div>
                  ...
                  <input
                    ...
                    value={item.title}
                    onChange={(e) =>
                      updateBatchItem(item.id, { title: e.target.value })
                    }
                  />
                </div>
                {currentCollection?.customFields.map((field) => (
                  <div key={field.id}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateBatchItemField(item.id, field.id, e.target.value)
                      }
                    />
                  </div>
                ))}
    ...
    <div className="text-center py-12 sm:py-20 space-y-4 sm:space-y-6">
      <div className="relative inline-block">
        ...
        <div className="relative bg-white p-4 sm:p-6 rounded-full shadow-lg border border-stone-100">
          <Sparkles
            size={32}
            className="sm:w-10 sm:h-10 text-amber-500 animate-pulse"
          />
        </div>
      </div>
      <div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`flex items-center justify-between p-4 sm:p-6 border-b ${borderClass}`}
        >
          <h2
